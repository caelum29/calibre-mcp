// Cover-board widget for the MCP Apps iframe (issues #19/#22). Visuals are the approved
// mockup (assets/cover-carousel.html) with the fake layer replaced by MCP plumbing:
// appInfo handshake, data pull via widget-initiated tools/call (Desktop strips
// structuredContent from the tool-result notification), click → calibre_get_book,
// Read → ui/open-link. SDK-free; served by server.ts as two ui:// resources (one per
// search tool, so the widget knows its tool without guessing). Injection hygiene: all
// book fields land via textContent/createElement — no innerHTML, no eval. The widget
// JS avoids template literals so this file's outer literal needs no escaping.
...
export function boardHtml(tool: "calibre_search" | "calibre_semantic_search", version: string): string {
  return TEMPLATE.replaceAll("__TOOL__", tool).replaceAll("__VERSION__", version);
}
...
<style>
/* ============ tokens — the ONLY place colors live (frozen mockup) ============ */
:root{
...
(function () {
  ...
  /* ================= widget state ================= */
  var w = document.getElementById("w"), strip = document.getElementById("strip"),
      pos = document.getElementById("pos"), prev = document.getElementById("prev"),
  ...
  function clearOpening() {
    clearTimeout(S._t);
    strip.querySelectorAll(".card.is-opening").forEach(function (c) { c.classList.remove("is-opening"); });
  }
  ...
  /* ================= boot: skeleton first, then handshake ================= */
  renderSkeleton();
  setState("loading");
