...
D-016 | 2026-07-15 | **Front-matter demotion (issue #18): flag chunks at INDEX time via the `detectChapters` first-chapter boundary (`frontMatterEnd`, guard: boundary > min(20%, 60k chars) → no front matter claimed); a chunk is front matter when the MAJORITY (≥50%) of it lies before the boundary; `calibre_semantic_search scope=book` stable-partitions front-matter hits below body matches AFTER the rerank stage (all modes; nothing dropped, `[front matter]` label + note). Schema change is ADDITIVE (`chunks.front_matter INTEGER DEFAULT 0`, guarded ALTER TABLE) — INDEX_VERSION stays 3, no full rebuild forced; books gain demotion on re-index. `calibre_search scope=book` (calibredb FTS, no offsets) gets NO auto-routing to the semantic pipeline — only description guidance + an in-band tip when the book is indexed.** | TOC/praise/foreword pages are keyword-dense but semantically empty, so they win exactly the definitional queries where body content is wanted (issue #18's real-session trace: 0 definitional passages across 3 canonical DDD books). Demote-don't-filter keeps "what does the foreword say" working; majority-overlap (not containment) because chunks run ~2k chars and the whole front matter typically lands in ONE boundary-straddling chunk a containment rule never flags (observed on the eval fixture: chunk @0-1906 vs boundary 1485). Auto-routing calibre_search would silently change its output shape/outputSchema contract. Validated per D-012 by the `front-matter-trap` eval kind (fixture 116): keyword-mode trap Hit@1 0.33→1.0 target, hybrid/vector non-regressed. Library-scope demotion (best-chunk-per-book selection can still surface a front-matter snippet) is a stated DEFERRAL. | This session (2026-07-15); GitHub issue #18; eval reports 2026-07-15-*-issue18-baseline / -issue18-after; `src/domain/structure/chapters.ts`, `src/semantic/store.ts`, `src/tools/calibre_build_index.ts`, `src/tools/calibre_semantic_search.ts`, `src/tools/calibre_search.ts`.
