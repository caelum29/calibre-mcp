...
extra tools (see `docs/dev/TOOLS-spec.md`).
...
`docs/dev/RESEARCH.md` §5.0 for the verified inventory and the coverage table.

**Tool-count target: keep the model-facing surface ≤ ~20 task/intent tools.** Field + research
evidence (`docs/dev/DESIGN.md` §9.1): selection accuracy degrades as the number of *confusable* tools per
...
> `docs/dev/DECISIONS.md` D-005 (rationale) and `docs/dev/TOOLS-spec.md` (build list of record).
